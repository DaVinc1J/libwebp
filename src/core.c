...
void create_instance(_app *p_app) {
	...
	VkApplicationInfo app_info = {
		...
		.pApplicationName = p_app->config.title,
		.applicationVersion = VK_MAKE_VERSION(1, 0, 0),
		.pEngineName = p_app->config.title,
...
bool8 is_physical_device_extensions_supported(VkPhysicalDevice physical_device) {
...
void create_logical_device(_app *p_app) {
	...
	bool8 same_family = (unique_queue_families[0] == unique_queue_families[1]);
...
bool8 is_physical_device_suitable(_app *p_app, VkPhysicalDevice physical_device) {
...
bool8 check_swapchain_support(_app *p_app, VkPhysicalDevice physical_device, _swapchain_support *p_support) {
